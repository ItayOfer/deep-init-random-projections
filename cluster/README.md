...
| 10 | [`10_rc_frozen_ends/`](10_rc_frozen_ends/README.md) | Jul 17–18, cont. Aug 15 | 100L, train only last3 (fc98,fc99,fc100) vs only first3 (fc1,fc2,fc3) — head always frozen in both, everything else frozen too; raw (`row_centered_he`) vs corrected (rcfwd) recipe; W5 continuation adds `first2`/`last2` windows + the missing `rawrescale`/`fwdbal` recipe-ablation corners | Raw: both DEAD/STUCK. Corrected recipe resolves *why*: `last3` starts learning (raw death was a forward-scale artifact, not content death); `first3` still fails — healthy gradients under both recipes rule out gradient conditioning as its bottleneck. **W5 (2026-08-15): runner + 34 `.sub` files ready, locally verified (freeze mechanics, `_parse_label` round-trip, fwdbal blow-up asymmetry) — cluster submission pending (no SSH access this session)**
| 11 | [`11_relu_shift/`](11_relu_shift/README.md) | Aug 15 | Post-ReLU DC removal `a = relu(Wx) − c·rms(a)` on unconstrained He weights — the **activation-space dual** of row-centering — swept over `c`, vs `he` and `row_centered_he` baselines | Init-time story complete and now **closed-form**: per-layer forward gain `G(c) = √(1 − 2c/√π + c²)`, minimised at `c = 1/√π` where `G = r ≈ 0.8256` — *exactly* row-centering's gain, so exact DC removal costs the same per-layer gain as row-centering — **transferable, not impossible**: boosting the weights by `1/G(c)` restores `g_fwd≈1` and drives `g_bwd→1/r`, i.e. the shift family obeys the same gain-coupling lock (oracle correction, see FRONTIER). Kills dying neurons outright (0.000 dead vs He's 0.34–0.48). `c = 0.1–0.25` beats **both** baselines on distance-correlation-to-input at 30L; `c ≥ 0.7` gives the best geometry ever measured but sits at chance content. **30L smokes ran 2026-08-15:** level with He under robust test statistics (an apparent +6.6 pp was a final-epoch artifact — retracted); 100L end-to-end unmeasured (8/10 arms aborted under the plain recipe) — synthesis §4.3 |
| 12 | [`12_frozen_readout/`](12_frozen_readout/README.md) | Aug 15 | Freeze a 100L net at init, train ONLY `fc99`+`fc100` (head frozen), vary only the initialization — rank `he` / `row_centered_he` / rcfwd / post-ReLU shift `c=0.10,0.25,0.70` by how much class content a trained readout can recover at depth 100 | Originally motivated by campaign 10's 0.83 on a representation the probes called dead — a reading later **retracted** (the 0.83 was *train* accuracy; test is 0.1132, so the probes were right — capacity ≠ content). **Ran 2026-08-15:** He first at 30L and 100L on held-out readout accuracy; every arm at chance at 100L — synthesis §5.2 |
...
**10_rc_frozen_ends, W5 continuation (2026-08-15, on `main`)** — same runner, extended additively: `first2=[fc1,fc2]`/`last2=[fc99,fc100]` windows, `rawrescale` (`row_centered_he`+`grad_rescale`, (B)-only) and `fwdbal` (`row_centered_forward_balanced`, no rescale, (A)-only) recipes decomposing rcfwd's two interventions, a `--target-train-accuracy` flag, and a `rawrescale` LR ladder (control `1e-2` + `1e2,1e4,1e6,1e7`, analytically-matched target `≈2.09e6`). `_parse_label` rewritten to try recipe suffixes longest-first (the old `endswith("_rcfwd")` check would otherwise silently classify every non-`raw` label as `raw`, since `raw`'s suffix is empty — demonstrated by simulating the naive check: 64/80 labels mis-parsed) with a round-trip self-test over all 80 `EXPERIMENT_LABELS` run at import time. 34 new `.sub` files (4 target-accuracy `last2`/`last3`×rcfwd @ 400ep/target 0.99, 8 smoke `first2`/`last2`×raw/rcfwd, 20 rawrescale-ladder, 2 `last2`/fwdbal), all dry-run-verified through the real argparse→`_build()` pipeline with training stubbed out. Local CPU checks (freeze mechanics, all-recipe construction, `--target-train-accuracy` plumbing) all pass; found that `last2`/fwdbal does **not** explode over 5 local CPU epochs (unlike `first2`/fwdbal, which NaNs immediately) — a discrepancy from the brief's stated expectation, documented in the campaign README rather than silently corrected. **No cluster jobs submitted this session (no SSH access)** — see campaign README "Reproduce → W5 continuation" for the exact handoff.
