...
| 10 | [`10_rc_frozen_ends/`](10_rc_frozen_ends/README.md) | Jul 17–18 | 100L, train only last3 (fc99,fc100,head) vs only first3 (fc1,fc2,fc3) — everything else frozen; raw (`row_centered_he`) vs corrected (rcfwd) recipe | Raw: both DEAD/STUCK. Corrected recipe resolves *why*: `last3` starts learning (raw death was a forward-scale artifact, not content death); `first3` still fails — healthy gradients under both recipes rule out gradient conditioning as its bottleneck |
