...
3. **Content-preserving initialization:** rcfwd closed the gradient-flow problem and thereby isolated the real frontier — an initialization whose *forward pass preserves class structure* at depth (the row-centered family provably doesn't). Candidate directions: partial centering (α trade-off), structure-preserving orthogonal hybrids, or accepting BN/architectural help — each measurable first with the campaign-01 geometry probes before any training.
