...
**Act 4 — the mechanism experiments (campaigns 10–12, 2026-08-15).** The motivation: campaign 09's probes said class content dies by depth ≈25 in the row-centered family — but a campaign-10 frozen window then reached 0.83 *train* accuracy on a representation those probes had called dead, which read at the time as "the probes understate badly; the class hierarchy survives depth after all." Campaigns 10–12 were built to settle that dispute: does a deep random stack at init genuinely destroy class structure, or is the structure merely undecodable by probes and unreachable by optimization? (The answer: the probes were right — the 0.83 was memorization; see capacity ≠ content below.) Concretely, three advisor asks, answered in `docs/reports/2026-08-15_campaign10_followup_synthesis.md`:
...
- Capacity ≠ content: **0.9498 train / 0.1132 test** (`rcfrozen_last3_audit` fmnist, 400 ep) — synthesis §4.1. What this means: a small trained head on a frozen 100L random stack can still tell the 60,000 training points apart well enough to memorize their labels — the map stays *injective*, so **capacity** survives — while test accuracy never leaves chance, because nothing **class-aligned** survives to generalize from. Two different properties of a representation, and train accuracy measures only the first; reading it as the second is what caused two of the four retractions.
