...
|---|---|---|---|---|
| W1 | **Campaign 10 — rc frozen ends** (100L `row_centered_he`; train only last-3 vs only first-3 layers) | **scaffolded — jobs not yet submitted** | Branch `work/rc-frozen-ends` has the `trainable_layers` config/classifier support (freeze mechanics verified locally on CPU), `cluster/10_rc_frozen_ends/` (runner + 8 subs), campaign README. Blocked on cluster access from this session (no passwordless SSH) — next agent/terminal with cluster creds: `bash cluster/sync_to_cluster.sh` → clear `__pycache__` → `sbatch` the 4 smoke subs → pull → triage → gate audits | The brief; branch `work/rc-frozen-ends`; `cluster/10_rc_frozen_ends/README.md` |
