...
|---|---|---|---|---|
| W1 | **Campaign 10 — rc frozen ends** (100L `row_centered_he`; train only last-3 vs only first-3 layers) | **worked — awaiting oracle merge** | All 4 smoke jobs ran and were pulled: both `last3` cells **DEAD** (fc99/fc100 gradients underflow to exact float32 zero by ep3–8), both `first3` cells **STUCK** (fc1–fc3 gradients healthy, order 1–5, but zero loss movement over 20 ep — absorbed by 97 frozen downstream layers). Refutes the advisor's "both train fast" bet; confirms the oracle's `last3` prediction exactly and sharpens it (exact zero, not just tiny); refutes the "slow" half of the `first3` prediction (inert, not slow). Audits deliberately not gated — no evidence of progress to extrapolate from. Branch `work/rc-frozen-ends` has everything (code, 4 JSONs, figure, docs); oracle to spot-check the headline numbers, merge, and (only after that) fold into `docs/RESEARCH_LOG.md` | The brief (Outcome section filled); `cluster/10_rc_frozen_ends/README.md` §Findings (incl. `docs/figures/rcfrozen_mechanisms.png`); `reports/results/rcfrozen_*_smoke_*_100L.json` |
