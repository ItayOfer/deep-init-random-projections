...
| rc frozen ends | `rcfrozen_{first3,last3}_smoke_{fmnist,cifar10}_100L[_rcfwd].json` (8) | `cluster/10_rc_frozen_ends/` | Jul 17–18 | 100L, train only last3 (fc99,fc100,head) vs first3 (fc1,fc2,fc3), rest frozen; two recipes. Raw (`row_centered_he`): both DEAD/STUCK — `last3` gradient underflows to exact float32 zero, `first3` healthy grads but zero loss movement. Corrected recipe (`row_centered_forward_balanced`+`grad_rescale`, campaign 09's recipe): `last3` **starts learning** (11%→21%/19% train acc, 20 ep, no plateau yet — the raw death was a forward-scale artifact, not content death); `first3` still fails, loss now *worsens* past ln(10) despite healthy gradients under both recipes (content/reachability bottleneck, not gradient conditioning). Audits not gated except `last3`/rcfwd flagged as a real candidate |
