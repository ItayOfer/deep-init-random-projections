#!/usr/bin/env python3
"""Same as he_funnel100_fwd_bwd_100L.png but with plain ROW-CENTERED He init
(row_centered_he, INITIALIZERS.md section 5) -- NOT V2.

row_centered_he: draw He weights, then subtract each row's mean (so every
output neuron's weights sum to zero). No variance adjustment, no per-layer
eta schedule.

He init, 100L, at initialization, forward/backward decomposition per layer:
...
for col, dataset in enumerate(["fashion_mnist", "cifar10"]):
    for row, (widths, name) in enumerate(
        ...
        ax.set_yscale("log")
        ax.set_title(f"{dataset} | row_centered_he {name}\n"
...
fig.suptitle("ROW-CENTERED He (row_centered_he), 100L at init: forward rms(A) vs backward "
             "rms(delta) vs grad row norm -- constant vs funnel 500->100", fontsize=13, y=1.01)
fig.tight_layout()
out = "reports/figures/gain_funnels/rc_funnel100_fwd_bwd_100L.png"
